"""Standalone runner for the feedback learning loop.

Follows the same pattern as ``operational_continuity_runner.py`` so the
nightly learning cycle can execute in web-only mode (no Discord required).
"""
...
# Default: run once every 24 hours (86 400 s).
_DEFAULT_INTERVAL = 86_400
...
class FeedbackLearningScheduler:
    """Background runner for the nightly feedback learning cycle."""

    job_name = "feedback_learning_cycle"

    def __init__(self, db: Any, *, interval_seconds: int = _DEFAULT_INTERVAL):
        self.db = db
    ...
    async def run_once(self, *, triggered_by: str = "schedule") -> dict[str, Any]:
        ...
        try:
            ...
            loop = FeedbackLearningLoop(self.db)
    ...
    async def _loop(self) -> None:
        while not self._stop.is_set():
            try:
                ...
                logger.warning("Feedback learning scheduler run failed: %s", exc)
                try:
                    if hasattr(self.db, "record_job_run"):
                        fail_key = _run_key()
